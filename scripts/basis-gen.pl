...
# Generate interface file for SML#
sub gen_randgen_smi
{
	my ($smi, $impl, $ver) = @_;
	my $type_stream_decl;
	if ($ver eq "3.5.0") {
		$type_stream_decl = "type stream (= *)";
	} else {
		$type_stream_decl = "type stream (= boxed)";
	}
...
_require "GENERATOR_SIG-${impl}.sml"
_require "Rand.smi"
